...
export default function Page({}: PageProps) {
    ...
    const notificationFiltered = useMemo(() => {
        ...
        if (filterParam && filterParam !== 'all') {
            items = items.filter((n) => n.category === filterParam);
        }
    ...
    const notificationCategories = useMemo(() => data?.notification_categories || [], [data?.notification_categories]);

    const handleMarkAsRead = useCallback(
        (id: string) => {
            db.transact(db.tx.notifications[id].update({ readAt: new Date() }));
        },
        [db],
    );
    ...
    return (
        <SidebarProvider>
            <AppSidebar />
            <AppContent className="space-y-4" header={!isMobile}>
                {/* Header */}
                <Card className="py-2">
                ...
                {isLoading ? (
                    ...
                    <p className="text-destructive text-center">Error: {String(error)}</p>
                ) : (
                    <>
                        {/* Filter Dialog */}
                                    ...
                                    <Table>
                                        <TableBody>
                                            {notifications.map((n) => {
                                                ...
                                                const IconComp =
                                                    iconOptions.find((i) => i.name === syncIcon)?.icon || Star;
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <TableCell className="w-1/2 max-w-0 truncate">
                                                            {n.body}
                                                        </TableCell>
